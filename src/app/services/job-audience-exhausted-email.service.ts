...
@Injectable()
export class JobAudienceExhaustedEmailService {
  ...
  async notifyAudienceExhausted(job: {
    id: number
    job_code: string
    job_title: string
    ...
    // Step 1: Guard -- department required
    if (!job.required_department_id) {
      this.customLogger.warn(`[JobAudienceExhaustedEmail] Job ID ${job.id} has no required_department_id -- skipped`)
      return
    }

    // Step 2: Fetch department + facility
    const dept = await this.prismaService.departments.findUnique({
      where: { id: job.required_department_id },
    })
    if (!dept) {
      this.customLogger.warn(
        `[JobAudienceExhaustedEmail] Department ID ${job.required_department_id} not found for job ID ${job.id} -- skipped`,
      )
      return
    }

    const facility = await this.prismaService.facilities.findUnique({
      where: { id: dept.facility_id },
    })
    if (!facility) {
      this.customLogger.warn(
        `[JobAudienceExhaustedEmail] Facility ID ${dept.facility_id} not found for job ID ${job.id} -- skipped`,
      )
      return
    }

    // Step 3: Find Partner Admin emails of the facility
    const adminEmails = await this.getPartnerAdminEmails(dept.facility_id)
    if (adminEmails.length === 0) {
      this.customLogger.warn(
        `[JobAudienceExhaustedEmail] No Partner Admin emails found for facility ID ${dept.facility_id} (job ID ${job.id}) -- skipped`,
      )
      return
    }

    // Step 4: Fetch supervisor name
    ...
    // Step 6: Call notification service email API
    try {
      await firstValueFrom(
        this.httpService
          .post(
            ...
            {
              emailEventName: EmailEventName.JOB_AUDIENCE_EXHAUSTED_EVENT,
              payload: {
                to: adminEmails,
                jobId: job.id,
      ...
      this.customLogger.log(`[JobAudienceExhaustedEmail] Email sent for job ID ${job.id} to ${adminEmails.join(', ')}`)
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      this.customLogger.error(`[JobAudienceExhaustedEmail] Failed to send email for job ID ${job.id}: ${errorMessage}`)
      // Do not re-throw -- caller (auto-switch batch) must continue
    }
  }

  private async getPartnerAdminEmails(facilityId: number): Promise<string[]> {
    // Select both plain and encrypted fields -- legacy records may have only email_encrypted
